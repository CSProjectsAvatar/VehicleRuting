...
%-----------------------------------------------------------------------------------
	...
	de vecindad añadiendo a $V$ un CVS aleatorio, % @audit el criterio no es aleatorio
	como se explicó anteriormente, y se 
	repite el proceso. El algoritmo se detiene cuando, después de $ N $ iteraciones, no
	encuentra una solución mejor.

	\begin{algorithm}[htb]\label{algor:IVNS} % @audit habla con Omar xq el algoritmo ta mal
		\caption{Snake IVNS}
		...
		$ V \la \langle \rangle $\;
		\While{$ k \leq N $}{
			$v \la $ CVS aleatorio\; % @audit no es aleatorio
			$V \gets V + v$\;
...
%-----------------------------------------------------------------------------------
	...
	% @todo actualizar to estos resultados
	% ----------------------- 33 5 ---------------------------------
	\begin{figure}[htbp]
		\centering
